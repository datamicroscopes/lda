...
std::vector<T> selectByIndex(const std::vector<T> &v, const std::vector<size_t> &index )  {
    ...
    for(size_t tid: index){
        new_v.push_back(v[tid]);
    }
...
public:
    ...
    state(const model_definition &def,
        ...
        V = def.v();
        rng_ = rng;
        for(size_t tid = 0; tid < x_ji.size(); ++tid) {
            using_t.push_back({0});
        }
        dishes_ = {0};

        for(size_t eid = 0; eid < x_ji.size(); ++eid) {
            k_jt.push_back({0});
            n_jt.push_back({0});

            n_jtv.push_back(std::vector< std::map<size_t, size_t>>());
            for (size_t t = 0; t < using_t[eid].size(); ++t)
            {
        ...
        for(size_t tid=0; tid < docs.size(); tid++){

            t_ji.push_back(std::vector<size_t>(docs[tid].size(), 0));
        }
    }
    ...
    */
    ...
    inference(){
        for (size_t eid = 0; eid < x_ji.size(); ++eid){
            for (size_t tid = 0; tid < x_ji[eid].size(); ++tid){
                sampling_t(eid, tid);
            }
        }
        for (size_t eid = 0; eid < x_ji.size(); ++eid){
            for (auto t: using_t[eid]){
                if(t != 0) {
                    sampling_k(eid, t);
                }
            }
    ...
    docDist(){
        ...
        for(size_t tid = 0; tid < am_k.size(); ++tid) {
            am_k[tid] *= alpha_ / sum_am_dishes_;
        }

        for(size_t eid = 0; eid < k_jt.size(); eid++){
            std::vector<size_t> &n_jt_ = n_jt[eid];
    ...
    perplexity(){
        ...
        for(size_t eid = 0; eid < x_ji.size(); eid++){
            ...
            for(auto &v: py_x_ji){
                double word_prob = 0;
                for(size_t tid = 0; tid < p_jk.size(); tid++){
                    auto p = p_jk[tid];
                    auto &p_kv = phi[tid];
                    word_prob += p * p_kv[v];
                }
...
// private:
    ...
    sampling_t(size_t eid, size_t tid){
        ...
        if (t_new == 0)
        {
            ...
            t_new = create_table(eid, k_new);
        }
        seat_at_table(eid, tid, t_new);
    }

    void
    sampling_k(size_t eid, size_t t){
        leave_from_dish(eid, t);
        std::vector<float> p_k = calc_dish_posterior_t(eid, t);
        validate_probability_vector(p_k);
        ...
        seat_at_dish(eid, t, k_new);
    }

    void
    leave_from_dish(size_t eid, size_t t){
        size_t k = k_jt[eid][t];
        assert(k > 0);
        assert(m_k[k] > 0);
    ...
    std::vector<float>
    calc_dish_posterior_t(size_t eid, size_t t){
        std::vector<float> log_p_k(dishes_.size());

        auto k_old = k_jt[eid][t];
        auto n_jt_val = n_jt[eid][t];
        for(size_t tid = 0; tid < dishes_.size(); tid++){
            auto k = dishes_[tid];
            if(k == 0) continue;
            float n_k_val = (k == k_old) ? get_n_k(k) - n_jt[eid][t] : get_n_k(k);
            assert(n_k_val > 0);
            log_p_k[tid] = distributions::fast_log(m_k[k]) + distributions::fast_lgamma(n_k_val) - distributions::fast_lgamma(n_k_val + n_jt_val);
            assert(isfinite(log_p_k[tid]));
        }
        log_p_k[0] = distributions::fast_log(gamma_) + distributions::fast_lgamma(V * beta_) - distributions::fast_lgamma(V * beta_ + n_jt[eid][t]);

        for(auto &kv: n_jtv[eid][t]){
            auto w = kv.first;
            ...
            for(size_t tid = 0; tid < dishes_.size(); tid++){
                n_kw[tid] = get_n_kv(dishes_[tid], w);
                if(dishes_[tid] == k_jt[eid][t]) n_kw[tid] -= n_jtw;
                assert(tid == 0 || n_kw[tid] > 0);
            }
            n_kw[0] = 1; // # dummy for logarithm's warning
            for(size_t tid = 1; tid < n_kw.size(); tid++){
                log_p_k[tid] += distributions::fast_lgamma(n_kw[tid] + n_jtw) - distributions::fast_lgamma(n_kw[tid]);
            }
    ...
    void
    seat_at_dish(size_t eid, size_t t, size_t k_new){
        m += 1;
        m_k[k_new] += 1;

        size_t k_old = k_jt[eid][t];
        if (k_new != k_old)
        {
            assert(k_new != 0);
            k_jt[eid][t] = k_new;
            float n_jt_val = n_jt[eid][t];
    ...
    void
    seat_at_table(size_t eid, size_t tid, size_t t_new){
        t_ji[eid][tid] = t_new;
        n_jt[eid][t_new] += 1;

        size_t k_new = k_jt[eid][t_new];
    ...
    create_table(size_t ein, size_t k_new)
    {
        ...
        for (size_t tid = 0; tid < using_t[ein].size(); ++tid)
        {
            if (tid != using_t[ein][tid])
            {
                t_new = tid;
                break;
            }
    ...
    calc_dish_posterior_w(const std::vector<float> &f_k){
        ...
        for(size_t tid = 0; tid < dishes_.size(); ++tid) {
            p_k(tid) = m_k[dishes_[tid]] * f_k[dishes_[tid]];
        }
        p_k(0) = gamma_ / V;
        p_k /= p_k.sum();
    ...
    std::vector<float>
    calc_table_posterior(size_t eid, std::vector<float> &f_k){
        std::vector<size_t> using_table = using_t[eid];
        Eigen::VectorXf p_t(using_table.size());

        for(size_t tid = 0; tid < using_table.size(); tid++){
            auto p = using_table[tid];
            p_t(tid) = n_jt[eid][p] * f_k[k_jt[eid][p]];
        }
